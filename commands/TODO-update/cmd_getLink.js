...
module.exports.run = function ({ api, args, event, textFormat }) {
	...
	if (args.length > 0) {
		for (const index of args) {
			if (!parseInt(index)) {
				return api.sendMessage(textFormat('error', 'errOccured', `Index "${index}" is not a number`), threadID, messageID);
			}
			
			const num_index = parseInt(index);
			if (num_index < 1|| num_index > attachments.length) {
				return api.sendMessage(textFormat('error', 'errOccured', `Index "${num_index}" out of range.`), threadID, messageID);
			}
